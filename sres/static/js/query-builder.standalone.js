...
QueryBuilder.OPERATORS = {
    ...
    matches_regex:    { type: 'matches_regex',    nb_inputs: 1, multiple: false, apply_to: ['string'] },
    not_matches_regex:{ type: 'not_matches_regex',nb_inputs: 1, multiple: false, apply_to: ['string'] },
...
QueryBuilder.prototype.createRuleFilters = function(rule) {
    ...
    rule.$el.find(QueryBuilder.selectors.filter_container).html($filterSelect);
    console.log(QueryBuilder.selectors.rule_header + ' .sres-list-chooser-show-chooser');
    if (rule.$el.find(QueryBuilder.selectors.rule_header + ' .sres-list-chooser-show-chooser').length == 0) {
        rule.$el.find(QueryBuilder.selectors.rule_header).append('<button type="button" class="btn btn-xs btn-outline-secondary sres-list-chooser-show-chooser"><span class="fa fa-table"></span> Choose lists</button>');
    }
...
QueryBuilder.prototype.getFilterById = function(id, doThrow) {
    ...
    console.error('Undefined filter', id);
...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header"> \
    <div class="btn-group float-right group-actions"> \
      <button type="button" class="btn btn-xs btn-outline-success" data-add="rule"> \
        <i class="{{= it.icons.add_rule }}"></i> {{= it.translate("add_rule") }} \
      </button> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <button type="button" class="btn btn-xs btn-outline-success" data-add="group"> \
          <i class="{{= it.icons.add_group }}"></i> {{= it.translate("add_group") }} \
        </button> \
      {{?}} \
      {{? it.level>1 }} \
        <button type="button" class="btn btn-xs btn-outline-danger" data-delete="group"> \
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  <div class="rule-header"> \
    <div class="btn-group float-right rule-actions"> \
      <button type="button" class="btn btn-xs btn-outline-danger" data-delete="rule"> \
  ...
  <div class="rule-value-container w-50"></div> \
</div>';

QueryBuilder.templates.filterSelect = '\
{{ var optgroup = null; }} \
<select class="form-control sres-column-rereferenceable sres-column-rereferenceable-querybuilder-chosen" name="{{= it.rule.id }}_filter"> \
...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
    ...
    if (typeof filter.input == 'function') {
        ...
        switch (filter.input) {
            ...
            default:
                h += '<input class="form-control w-100" type="text" name="' + name + '"';
                if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
...
QueryBuilder.defaults({
    mongoOperators: {
        ...
        matches_regex:    function(v) { return { '$regex': '' + Utils.escapeRegExp(v[0]) }; },
        not_matches_regex:function(v) { return { '$regex': '(?!' + Utils.escapeRegExp(v[0]) + ')' }; },
...
QueryBuilder.regional['en'] = {
  ...
  "operators": {
    ...
    "between": "between (not inclusive of)",
    "not_between": "not between (not inclusive of)",
    ...
    "not_matches_regex": "does not match regular expression",
    "is_null": "is null",
