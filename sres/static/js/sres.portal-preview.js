/**
    Preview
**/
$(document).on('click', "#show_student_view", function() {
    window.location = ENV['PORTAL_PREVIEW_ENDPOINT_IDENTIFIED'].replace('__identifier__', encodeURIComponent($("#preview_identifier").val()));
});
$(document).on('click', "#preview_random_student", function() {
    window.location = ENV['PORTAL_PREVIEW_ENDPOINT_RANDOM'];
});
$(document).on('focus', "#preview_identifier", function() {
 ...
 $(document).on("keypress", "#preview_identifier", function(e) {
    if (e.which == 13) {
        $("#show_student_view").trigger("click");
        return false;
    }
});
...
$(document).ready(function(){
    $("#preview_identifier").autocomplete(
        ...
        //window.location = ENV['PORTAL_PREVIEW_ENDPOINT_IDENTIFIED'].replace('__identifier__', encodeURIComponent(suggestion.sid));
